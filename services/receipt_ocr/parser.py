...
def extract_and_translate_items(ocr_text: str) -> list[dict]:
    ...
    prompt = (
        "請將以下收據中「實際購買的商品明細」擷取為 JSON 陣列，商品名稱請務必翻譯成精準、道地的繁體中文（台灣習慣用語）：\n"
        "重要規則：\n"
        "1. 收據開頭的店名、設施名（例如：釧路市丹頂鶴自然公園、六花亭、7-Eleven 等）是店鋪/機構名稱，絕對不是購買的商品！除非明細內有明確的「入園料」、「門票」、「入場券」或「大人/小人」字樣，否則嚴禁把店名當成購買項目！\n"
        "2. 請仔細找出實際購買的商品行與金額（例如：「Aとうきび茶 ¥200」中，品項是 とうきび茶，即玉米茶，金額 200，數量 1）。\n"
        "3. 品名前方的分類碼/符號（如 A、B、◇、•、* 等）請去除。\n"
        "4. 知名專門店品項指引：\n"
        "   - 若為 WORKMAN Plus 等戶外服飾店：商品皆為服飾、毛巾、腰帶、襪子等（如 MEDIHEAL 是疲勞修復機能服；ふわふわフェイスタ 是蓬鬆洗臉毛巾；GIベルト 是GI帆布腰帶；ドライメッシュ 是乾爽網眼短襪；シン・呼吸するインナー 是呼吸透氣內衣）。\n"
        "   - 若為 六花亭 等北海道甜點店：マルセイアイスサンド 是丸成冰淇淋夾心三明治；サクサクパイ 是現烤酥脆派；醍醐 是醍醐生藍莓夾心蛋糕。\n"
        "   - 若為 景點/設施門票：個人 大人 是成人門票/全票。\n"
        "   - 若為 飲品：とうきび茶 是玉米茶。\n"
        "5. 嚴格禁止將稅額（消費税、対象計、内税、外税、税合計、軽減税率適用）、小計、合計、找零、店鋪資訊、電話、地址、法人登錄番號（登録番号）提取為商品！\n"
        "輸出格式必須是純 JSON 陣列，欄位如下：\n"
        "- \"name\": 繁體中文商品名稱 (例如：玉米茶)\n"
        "- \"originalName\": 收據上的原始名稱 (例如：とうきび茶)\n"
