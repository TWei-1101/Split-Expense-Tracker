...
test('每人花費摘要將總金額作為獨立欄位，不顯示筆數或統計範圍小字', () => {
  assert.doesNotMatch(appSource, /統計範圍：/);
  assert.doesNotMatch(appSource, /全部 \$\{expenses\.length\} 筆/);
  assert.doesNotMatch(appSource, /合計 TWD \{totalSpending\.toFixed\(0\)\}/);
  assert.match(appSource, /總金額<\/p>/);
