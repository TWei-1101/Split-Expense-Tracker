...
import { CircleDollarSign, Trash2, Plus, Minus, Users, X, Check, Pencil, UserPlus, UserMinus } from 'lucide-react';

// --- 全域變數和 Firebase 設定 ---
const appId = 'default-app-id';

// 這裡貼 Firebase Console 給你的設定
...
const initialAuthToken = null; // 先用匿名登入就好
...
const App = () => {
  ...
  // --- 2. 數據獲取 (Firestore 監聽) ---
  useEffect(() => {
    // 關鍵修復: 只有當 DB 初始化完成 AND 成功取得 userId (即通過身份驗證) 時才監聽資料
    ...
    const unsubscribeExpenses = onSnapshot(expensesQuery, (snapshot) => {
      const fetchedExpenses = snapshot.docs.map(doc => {
        const data = doc.data();
        // 處理 Firestore Timestamp 轉換為 JavaScript Date 物件
    ...
    const unsubscribeMembers = onSnapshot(membersDocRef, (docSnap) => {
        if (docSnap.exists()) {
            ...
            setDefaultSharesConfig(shares); // 儲存所有非預設值 1 的份數設定
        } else {
            setCustomMembers([]);
  ...
  // --- 3. 衍生狀態：計算最終成員清單 ---
  useEffect(() => {
    // 總成員清單：包含當前用戶 ID 和自訂成員
    let currentMembers = [userId].filter(Boolean); 
    
    // 過濾掉與 userId 重複的自訂成員，並加入清單
  ...
  const getInitialShares = useCallback(() => {
    return members.reduce((acc, name) => {
        // 使用 defaultSharesConfig 中的設定值，若無則預設為 1 (平均分配)
  ...
  // 當成員清單更新時，如果 Modal 關閉，重設新增表單的分帳份數
  useEffect(() => {
    if (!isExpenseModalOpen && userId) {
        setNewExpense(prev => ({
            ...prev,
            payerName: prev.payerName || userId,
            shares: getInitialShares(), // 使用預設份數初始化
        }));
    }
  ...
  const startAdd = () => {
    setEditingExpenseId(null);
    // 確保表單狀態為新的，並根據當前 members 列表初始化 shares
    setNewExpense({
      description: '',
      amount: '',
      payerName: userId,
  ...
  // 轉換 userId 到顯示名稱
  const getDisplayName = (member) => member === userId ? '我 (You)' : member;
  ...
  if (!authReady) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-50">
        <p className={`text-xl text-${primaryColor}-600`}>應用程式載入中...</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="p-8 bg-red-100 border border-red-400 text-red-700 rounded-lg m-4">
        ...
        <p>{error}</p>
        <p className="mt-2 text-sm">請檢查您的 Firebase 設定或網路連線。</p>
      </div>
    );
  }
